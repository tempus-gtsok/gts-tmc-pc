			...
			<div v-if="orgnames !=''">
				...
				<div class="tablists" v-if="tablist.length > 0 && organops == 1">
					<el-table :header-cell-style="{background:'#f6f6f6',color:'#666'}" border="" :data="tablist" tooltip-effect="dark" style="width: 100%">
						...
						<el-table-column label="适用范围" show-overflow-tooltip>
							<template slot-scope="scope">{{ statsname(scope.row.apprvCostFlowStaffs) }}</template>
						</el-table-column>
						<el-table-column label="操作" width="170px">
							<template slot-scope="scope">
								<el-button size="mini" @click="handleEdit(scope.$index, scope.row)">编辑</el-button>
								<el-button size="mini" v-if="scope.row.apprvCostFlowStaffs.length > 0" type="danger" @click="handleDelete(scope.$index, scope.row)">删除</el-button>
							</template>
						</el-table-column>
					</el-table>
				...
				<div class="orgts_two" v-if="organops == 2">
					...
					<div class="namstr">
						...
						<div>
							<el-select v-model="deptLevel" @change="deptlv" style="width: 200px;" placeholder="请选择">
								<el-option v-for="items in deptLevellist" :key="items.id" :label="items.name" :value="items.id">
								</el-option>
							</el-select>
						</div>
					</div>
			...
			<div class="trstp">
				<el-table :header-cell-style="{background:'#008CCC',color:'#ffffff'}" border="" ref="multipleTable" :data="userlists" tooltip-effect="dark" @select="handleSelection" @select-all="handleSelectionall"
				 @selection-change="handleSelectionChange" style="width: 100%">
					...
					<el-table-column label="成本中心" width="130px" show-overflow-tooltip>
...
<script>
	export default {
		...
		methods: {
			deptlv() { //修改部门审批触发
				if (this.deptLevel == 0) {
					this.plohslist[0].name = "开始";
				} else if (this.deptLevel == 1) {
					this.plohslist[0].name = "本部门";
				} else if (this.deptLevel == 2) {
					this.plohslist[0].name = "本部门 + 上级部门";
				} else if (this.deptLevel == 3) {
					this.plohslist[0].name = "本部门 + 上级部门 + 上上部门";
				}
			},
			async handleEdit(index, row) { //修改
				try {
					let res = await this.$api.business.getApprvCostDetail({
						id: row.id
					});
					if (res.code == 200) {
						this.isaids = row.id;
						...
						this.okuserlist = res.data.apprvCostFlowStaffs;
						let das = res.data.apprvCostFlowNodes;
						this.plohslist = [{
							name: '开始'
						}]
						for (let k in das) {
							this.plohslist.push({
								name: '节点',
								user: das[k].apprvCostFlowNodePersons,
								ra: das[k].nodeModel
							})
						}
						this.organops = 2;
			...
			handleDelete(index, row) { //删除当前流程
				this.$confirm('此操作将删除该审批流程, 是否继续?', '提示', {
					...
					this.loding = true;
					this.$api.business.deleteApprvCost({
						id: row.id
					}).then((res) => {
						this.loding = false;
						if (res.code == 200) {
			...
			async addsplki() { //新增修改当前流程
				...
				try {
					...
					for (let i = 0; i < plohslist.length; i++) {
						if (i != 0) {
							apprvCostFlowNodes.push({
								apprvCostFlowNodePersons: plohslist[i].user,
								nodeSeq: i,
					...
					if (this.isadd) {
						datsli = {
							apprvCostFlowStaffs: okuserlist,
							apprvCostFlowNodes: apprvCostFlowNodes,
							costId: this.costId,
							flowName: orgnameti,
					...
					let res = await this.$api.business.insertOrUpdateApprvCost(datsli)
					if (res.code == 200) {
						if (this.isadd) {
							this.$message({
								message: "新增流程成功",
			...
			async handleSelection(val, row) { //单选
				...
				if (selected) { // true就是选中，0或者false是取消选中
					if (that.ops == 1) {
						...
						let res = await that.$api.business.staffJudgeApprvCost(dat);
						if (res.code == 200) {
							if (res.data == true) {
								that.$message({
					...
					for (let k in that.userlists) {
						if (that.ops == 1) {
							if(sit == false){
								...
								that.$api.business.staffJudgeApprvCost(dat).then((res) => {
									if (res.code == 200) {
										if (res.data == true) {
											that.$message({
			...
			async raclicks(it) { //启用会关闭当前成本  审批流程
				let ins = 0;
				if (it) {
					ins = 0;
				} else {
				...
				this.loading = true;
				try {
					let res = await this.$api.business.switchFlowsCostApprv({
						on: ins,
						where: {
							flowType: this.ortyid,
							costId: this.costId
			...
			async searchtype() { //查询当前成本审批流程
				this.loading = true;
				try {
					this.organops = 1
					let res = await this.$api.business.apprvCostFlowList({
						flowType: this.ortyid,
						costId: this.costId
					})
					if (res.code == 200) {
			...
			async searchdept() { //查询所有成本中心
				...
				try {
					...
					let res = await this.$api.home.getCostCenterList();
					this.loading = false;
					if (res.code == 200) {
						this.deptis = [{
							id: 0,
